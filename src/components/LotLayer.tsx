...
export default function LotLayer() {
  ...
  // Обработчик кликов для режима drawLot
  ...
  return (
    <Group /* сам Group не блокирует события, блоком был Rect */>
      {/* ВАЖНО: кликабельный фон рендерим ТОЛЬКО в режиме drawLot */}
      {tool === 'drawLot' && W > 0 && H > 0 && (
        <Rect
          ...
          width={W}
          height={H}
          fill="rgba(0,0,0,0.001)"   // почти прозрачно, но кликабельно
          listening={true}           // слушаем события только сейчас
      ...
      {/* Существующие лоты */}
      {zone?.lots.map(l => {
        const active = l.lot_id === activeLotId;
        const pts = l.image_polygon.flatMap(p => [p.x, p.y]);
        return (
          <Group key={l.lot_id} onClick={() => selectLot(l.lot_id)}>
            <Line
              points={pts}
              closed
            ...
            {active && (tool === 'editLot' || tool === 'select') && l.image_polygon.map((p,i)=>(
              <Circle
                ...
                onDragMove={(e)=>{
                  ...
                  updateLot(zone.id, l.lot_id, { image_polygon: next });
                }}
              />
            ))}
          </Group>
      ...
      {/* Черновик лота (визуализация при drawLot) */}
      {tool === 'drawLot' && lotDraft && lotDraft.length>0 && (
        <Group listening={false}>
          <Line
            points={lotDraft.flatMap(p=>[p.x,p.y])}
            closed={false}
            stroke="#cbbdff"
            strokeWidth={1.5}
            dash={[6,6]}
